...
const drawerVariants = cva(
  ...
  ],
  {
    variants: {
      side: {
        right: 'top-0 right-0 h-full w-full max-w-md border-l border-line data-[state=closed]:translate-x-full data-[state=open]:animate-slide-in-right',
        ...
        bottom: 'bottom-0 left-0 right-0 max-h-[80vh] border-t border-line data-[state=closed]:translate-y-full data-[state=open]:animate-slide-in-up',
      },
    },
...
export const DrawerHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  function DrawerHeader({ className, ...rest }, ref) {
    return <div ref={ref} className={cn('flex flex-col gap-1.5 p-6 border-b border-line', className)} {...rest} />;
  },
);
...
>(function DrawerDescription({ className, ...rest }, ref) {
  return (
    <RadixDialog.Description ref={ref} className={cn('text-body-sm text-ink-3', className)} {...rest} />
  );
});
...
export const DrawerFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  function DrawerFooter({ className, ...rest }, ref) {
    return (
      <div
        ref={ref}
        className={cn('mt-auto flex flex-col-reverse gap-2 border-t border-line p-6 sm:flex-row sm:justify-end', className)}
        {...rest}
      />
    );
  },
